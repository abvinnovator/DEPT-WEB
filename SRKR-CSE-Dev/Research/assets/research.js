...
window.onload=function(){
    var elem=document.getElementById('rc-c');
    console.log(elem);
    elem.classList.add('research-active');
}
const observer=new IntersectionObserver((entries)=>{
    entries.forEach((entry)=>{
        if(entry.isIntersecting){
            entry.target.classList.add('research-active');
        }
    });
});
const hid=document.querySelectorAll('.research-container');
